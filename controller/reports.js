...
exports.createReport = async (req, res) => {
  ...
  if (!req.body.reportDetails || !req.body.reportDetails.userID) {
    return res
      .status(400)
      .send({ message: 'Please fill in the report details' });
  }
  ...
  try {
    ...
    if (report) {
      ...
      await report.update({
        userID: userID ? userID : report.userID,
        marketID: marketID ? marketID : report.marketID,
        marketName: marketName ? marketName : report.marketName,
        marketType: marketType ? marketType : report.marketType,
        cmdtyName: cmdtyName ? cmdtyName : report.cmdtyName,
        priceUnit: priceUnit ? priceUnit : report.priceUnit,
        convFctr: convFctr,
        price: price,
      });
    } else {
